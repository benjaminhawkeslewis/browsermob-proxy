...
public class BrowserMobHttpClientIT {
	 ...
	 * <code>&lt;h1&gt;Hello World&lt;/h1&gt;</code> on <code>/html</code> and
	 * a 1x1 GIF on <code>/pixel</code>.
	 */
	@ClassRule
    public static ExternalResource embeddedServer = new ExternalResource() {
		 ...
		 * Return a free port
		 * 
		 * Adapted from Aviad Ben Dov's recipe
		 * <a href="http://chaoticjava.com/posts/retrieving-a-free-port-for-socket-binding/">Retrieving a Free Port for Socket Binding</a>
		 * (Chaotic Java, 2010-06-07)
		 * 
		 * @throws IOException If there's an error opening a exploratory socket.
		 * @throws InterruptedException If the thread was interrupted while
		 *    closing the exploratory socket.
		 * @return A free port
		 */
	...
	public void canCaptureResponseBodyInHarLog() {
		...
		BrowserMobHttpRequest request = client.newGet("http://127.0.0.1:" + port + "/html?foobar=baz");
		BrowserMobHttpResponse response = client.execute(request);
